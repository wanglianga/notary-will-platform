...
>
> 修复本轮新增功能相关的角色页面可用性问题：公证员工作台不得使用后端不存在的 IN_PROGRESS 状态查询，应改为后端支持的办理状态或统一状态映射；档案室前端路由和登录跳转角色应与后端 ARCHIVIST 保持一致，确保 archivist1 能进入档案室页面。修复后重新验证高风险谈话、补件有效期/版本对比、收费窗口重谈费用判断相关页面操作链路。
>
> 继续修复新增功能回归问题：补件版本对比应调用按案件查询的 /materials/supplement-versions 或传入真实 supplementItemId，不能向 Long 路由传 current；收费员工作台应适配 /api/cashier/statistics 返回的 totalPaid/totalUnpaid/totalRefunded 字段或提供默认数值，避免 formatCurrency(undefined) 运行时错误。修复后重新验证审核员补件版本对比和收费窗口重谈费用判断页面。
